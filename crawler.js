...
async function main() {
  try {
    ...
    const HG_RTN_TOP_200 = {
      packName: "HG, RTN 포함 Top Price 200 스페셜팩 (10강, 95+)",
      playerPrice: [],
    };
    const UT_TOP_1500 = {
      packName: "UT 포함 Top Price 1500 스페셜팩 (4~8강, 103+)",
      playerPrice: [],
    };
    ...
    // -------------------------------------- HG, RTN, BTB, CAP, EBS, BOE21_TOP_200--------------------------------------

    const HG_RTN_LIST = await playerSearch([283, 284, 256, 252, 251, 253], 95); // playerSearch(시즌넘버, 최소오버롤)
    let HG_RTN_RESULTS = await playerPriceValue(HG_RTN_LIST, 10); // playerPriceValue(데이터 , 강화등급)
    await saveToDB(HG_RTN_RESULTS);
    const HG_RTN_FINAL = SortAndSlice(HG_RTN_RESULTS, 200); // SortAndSlice(데이터 , 자르기숫자)

    for (let item of HG_RTN_FINAL) {
      const playerDocs = await Price.find({ id: item.id });
      if (playerDocs.length > 0 && playerDocs[0]._id) {
        const playerData = {
          grade: item.prices.grade,
        ...
        HG_RTN_TOP_200.playerPrice.push(playerData);
      }
    }
    data.seasonPack.push({ ...HG_RTN_TOP_200 });
    // -------------------------------------- UT, 24KB, JNM, 24HEROES, DC, JVA, CC, FCA, 23HW, HG, RTN, 23HEROES, RMCF, LN, SPL, 23NG, LOL, FA, 23KFA, 22HEROES, BTB, CAP, CFA, EBS, BOE21, NTG, UP, 22KFA_TOP_800--------------------------------------

    const UT_LIST = await playerSearch(
      [
        814, 830, 813, 811, 802, 801, 289, 290, 291, 283, 284, 281, 274, 268,
        270, 804, 265, 264, 806, 261, 256, 252, 254, 251, 253, 249, 246, 293,
      ],
      103
    ); // playerSearch(시즌넘버, 최소오버롤)
    let UT_RESULTS = await playerPriceValue(UT_LIST, [4, 5, 6, 7, 8]); // playerPriceValue(데이터 , 강화등급)
    await saveToDB(UT_RESULTS);
    const UT_FINAL = SortAndSlice(UT_RESULTS, 1500); // SortAndSlice(데이터 , 자르기숫자)

    for (let item of UT_FINAL) {
      const playerDocs = await Price.find({ id: item.id });
      if (playerDocs.length > 0 && playerDocs[0]._id) {
        const playerData = {
          grade: item.prices.grade,
        ...
        UT_TOP_1500.playerPrice.push(playerData);
      }
    }
    data.seasonPack.push({ ...UT_TOP_1500 });
    // -------------------------------------- LN_TOP_85--------------------------------------
